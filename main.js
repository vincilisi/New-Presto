document.addEventListener("DOMContentLoaded", () => {
    ...
    reviws.forEach((recensione)=>{
        ...
        div.innerHTML = `
        <div class = "card"> <p class="lead text-center">${recensione.description}</p>
    ...
    stars.forEach((star, index)=>{
        ...
        for(let i = 0; i < reviws[index].rank; i++){ // Correggi: da i=1 a i=0 per includere anche il rank 1
            let icon = document.createElement(`i`);
            icon.classList.add(`fa-solid`, `fa-star`);
            star.appendChild(icon)
        }
        
        // Correggi: la differenza deve essere calcolata correttamente
        // Se rank = 5, difference = 0. Se rank = 2, difference = 3.
        let difference = 5 - reviws[index].rank;
        
        for(let i = 0; i < difference; i++){ // Correggi: da i=1 a i=0
            let icon = document.createElement(`i`);
    ...
    // INIZIALIZZA SWIPER QUI, DOPO CHE LE SLIDE SONO STATE AGGIUNTE AL DOM
